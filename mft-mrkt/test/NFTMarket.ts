...
describe("NFTMarket", () => {
  it("Should do something", async () => {
    ...
    const tokenURI = 'https://z.com';
    const trX = await nftMarket.createNFT(tokenURI);
    const reciept = await trX.wait();

    // Assert that the newlt created NFT's token URI is the same one
    const tokenID = reciept.events[0].args.tokenId;
    const mintedTokenURI = await nftMarket.tokenURI(tokenID)
    expect(mintedTokenURI).to.equal(tokenURI)

    // Assert that the newlt created NFT's signer is the owner
    const owner = await nftMarket.ownerOf(tokenID)
    const signers = await ethers.getSigners();
    const currentAddress = await signers[0].getAddress();
    expect(currentAddress).to.equal(currentAddress);
  })
})
